<?php
/**
 * @Last Modified time: 2014-11-14 22:04:50
 */
...
class RestServer {
   ...
   */
  public function  __construct($mode = 'debug', $realm = 'Serveur Rest') {
    $this->mode  = $mode;
    $this->realm = $realm;
  ...
  protected function findUrl() {
    ...
    foreach ($urls as $url => $call) {
      ...
      if (!strstr($url, '$')) {
        ...
        if (@preg_match(":^$regex$:", urldecode($this->url), $matches) OR urldecode($this->url) == "") {
          $params = array();
          $paramMap = array();
          if (isset($args['data'])) {
            $params[$args['data']] = $this->data;
  ...
  public function getData() {
    ...
    if ($this->format == RestFormat::AMF) {
      ...
      if ($ndata == NULL) parse_str($data, $ndata);
      $data = json_decode(json_encode($ndata));
    }
  ...
  function array2xml($array, $node_name = "document") {
        ...
        foreach ($mixed as $index => $mixedElement) {
          if (is_int($index)) {
            if ($index === 0) {
              ...
              $node = $DOMDocument->createElement($domElement->nodeName);
